...
from sa3d.sa3d_mult_field import  MultiObjectMaskField
...
class TCNNMaskField(Field):
    ...
    def __init__(
        ...
        self.num_obj = 8
        # SWT
        self.mask_field = MultiObjectMaskField(self.num_obj, num_levels, features_per_level, log2_hashmap_size, base_res, growth_factor)
    """
    SWT
        添加位置偏移使mask weight不同
    """
    def modify_positions_by_obj_id(self, positions_flat, obj_id):
        # 为每个目标添加偏移，使其坐标独立
        offset = torch.tensor([obj_id * 0.01, obj_id * 0.01, obj_id * 0.01], device=positions_flat.device)
        obj_positions_flat = positions_flat + offset
        return obj_positions_flat
    ...
    def get_outputs(self, ray_samples: RaySamples, weights: TensorType) -> TensorType:
        ...
        positions_flat = positions.view(-1, 3) # [num_rays * num_samples, 3]
        """
            positions_flat, tensor([[0.6004, 0.3021, 0.5101],
            [0.5910, 0.3047, 0.5117],
            [0.5817, 0.3072, 0.5133],
            ...,
            [0.3200, 0.9683, 0.5753],
            [0.3102, 0.9815, 0.5739],
            [0.2999, 0.9951, 0.5722]], device='cuda:0')
        """
        mask_weights = self.mask_grids(positions_flat).reshape(*weights.shape[:2], -1) #[num_rays, num_samples, num_levels]
        mask_weights = (mask_weights * self.level_alphas.view(1,1,-1).to(weights.device)).sum(-1, keepdim=True)  # [num_rays, num_samples, 1]
        mask_scores = (mask_weights * weights).sum(1) #[num_rays, 1]

        # 添加多目标
        # 计算多目标的 mask_weights
        mask_weights = self.mask_field.get_multi_object_mask_weights(positions_flat, weights, self.num_obj) # 16384, 48, 16, 3])
        mask_weights = (mask_weights * self.level_alphas.view(1, 1, -1 ,1).to(weights.device)).sum(dim=2) # [16384, 48, 3]
        mask_scores = (mask_weights * weights).sum(dim=1)  # [num_rays, 3]
        return mask_scores
